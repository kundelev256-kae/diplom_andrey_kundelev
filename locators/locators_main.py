...
class MainPage(WebPage):
    ...
    # === Career guidance form (форма профориентационного теста) ===
    ...
    # Поле ввода имени — ищет по атрибуту name (first_name/name) или placeholder
    ...
    email_form_input = WebElement(xpath="//input[@name='email' or @type='email' or @placeholder='* Ваш e-mail']")
    # Поле ввода телефона — ищет по name, типу input (tel) или placeholder
    phone_form_input = WebElement(xpath="//input[@name='phone' or @type='tel' or @placeholder='* Телефон']")
    # Кнопка отправки формы — ищет по типу submit или тексту "Отправить"/"Готово"
    submit_form_btn = WebElement(xpath="//button[@type='submit' or contains(text(),'Отправить') or contains(text(),'Готово')]")
